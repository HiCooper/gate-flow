 ...
 * });
 */
export function useGateFlowEvents(
  { handlerId: trackedHandlerId, ...callbacks }: GateFlowEventCallbacks = {},
): void {
  const callbacksRef = useRef<GateFlowEventCallbacks>(callbacks)
